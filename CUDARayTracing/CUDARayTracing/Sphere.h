...
public:
	...
	bool findIntersection(Ray ray, XMFLOAT3& intersection_point) {
		...
		float dist = ((-b - sqrt(discriminant)) / 2) - 0.000001;

		if (dist < 0)
			dist = ((-b + sqrt(discriminant)) / 2) - 0.000001;

		XMStoreFloat3(&intersection_point, XMLoadFloat3(&ray.origin) + dist * XMVector3Normalize(XMLoadFloat3(&ray.direction)));
